...
export class TabNavItem {
  ...
  registerdndEvents() {
    ...
    this.tabs.plugin.registerDomEvent(this.tabitemEl, "drop", (e: DragEvent) => {
      ...
      let dropIndex = (this.tabs.tabsConfig.titlePosition === "top" || this.tabs.tabsConfig.titlePosition === "bottom") ?
        e.clientX - this.tabitemEl.getBoundingClientRect().left < this.tabitemEl.clientWidth / 2 ? this.index : this.index + 1 :
        e.clientY - this.tabitemEl.getBoundingClientRect().top < this.tabitemEl.clientHeight / 2 ? this.index : this.index + 1;
      ...
      if (tabDragger.fromTabs.context.sourcePath === tabDragger.toTabs.context.sourcePath && tabDragger.fromTabs.activeView === tabDragger.toTabs.activeView) {
        ...
        if (tabDragger.fromTabs === tabDragger.toTabs) {
          ...
          this.tabs.activeView?.editor.replaceRange(newDoc,
        ...
        // 不同 nav 之间的拖拽
          ...
          const tabDragger = this.tabs.plugin.tabDragger;
          tabDragger.draggedTab = new TabNavItem(tabDragger.toTabs.tabsNav, tabDragger.dropIndex, tabDragger.draggedTab.title);
          tabDragger.draggedContent = new TabContent(tabDragger?.dropIndex, tabDragger.draggedTab.title, tabDragger.draggedContent.content, tabDragger.toTabs.app, tabDragger.toTabs.context);
          ...
          if (dropIndex === tabDragger.toTabs.tabsNav.navItems.length - 1) {
            ...
            tabDragger.toTabs.tabsNav.navWrapperEl.insertBefore(tabDragger.draggedTab.tabitemEl, tabDragger.toTabs.tabsNav.navItems[this.tabs.plugin.tabDragger.dropIndex + 1].tabitemEl);
            tabDragger.toTabs.tabsContents.tabcontentsEl.insertBefore(tabDragger.draggedContent.contentEl, tabDragger.toTabs.tabsContents.tabcontents[tabDragger.dropIndex + 1].contentEl);
          }
          // 增加点击事件
          ...
          if (this.tabs.plugin.settings.doubleClickToEdit) {
            tabDragger.toTabs.plugin.registerDomEvent(tabDragger.draggedContent.contentEl, "dblclick", (e: MouseEvent) => {
              e.preventDefault();
          ...
          for (let i = 0; i < tabDragger.toTabs.tabsNav.navItems.length; i++) {
            toTabsNewDoc += tabDragger.toTabs.split + tabDragger.toTabs.tabsNav.navItems[i].title + "\n" + tabDragger.toTabs.tabsContents.tabcontents[i].content;
          }
          tabDragger.toTabs.updateBackquote(tabDragger.draggedContent.content);
          toTabsNewDoc = this.tabs.backquote.repeat(tabDragger.toTabs.backquoteCount) + 'tabs\n' + tabDragger.toTabs.tabsConfig.rawConfig + "\n" + toTabsNewDoc + this.tabs.backquote.repeat(tabDragger.toTabs.backquoteCount);
          ...
          tabDragger.fromTabs.tabsContents.tabcontentsEl.removeChild(tabDragger.fromTabs.tabsContents.tabcontents[dragIndex].contentEl);
          ...
          let fromTabsNewDoc = "";
          for (let i = 0; i < tabDragger.fromTabs.tabsNav.navItems.length; i++) {
            fromTabsNewDoc += tabDragger.fromTabs.split + tabDragger.fromTabs.tabsNav.navItems[i].title + "\n" + tabDragger.fromTabs.tabsContents.tabcontents[i].content;
          }
          fromTabsNewDoc = tabDragger.fromTabs.backquote.repeat(this.tabs.backquoteCount) + 'tabs\n' + tabDragger.fromTabs.tabsConfig.rawConfig + "\n" + fromTabsNewDoc + tabDragger.fromTabs.backquote.repeat(tabDragger.fromTabs.backquoteCount);
          ...
          if (this.tabs.plugin.tabDragger.fromTabs.context.sourcePath === this.tabs.plugin.tabDragger.toTabs.context.sourcePath) {
            ...
            if (tabDragger.toTabs.sectionInfo.lineStart > tabDragger.fromTabs.sectionInfo.lineEnd) {
              // 从上到下
              activeView.editor.replaceRange(toTabsNewDoc,
                { line: tabDragger.toTabs.sectionInfo.lineStart, ch: 0 } as EditorPosition,
                { line: tabDragger.toTabs.sectionInfo.lineEnd, ch: tabDragger.toTabs.activeView?.editor.getLine(tabDragger.toTabs.sectionInfo.lineEnd).length } as EditorPosition);
              activeView.editor.replaceRange(fromTabsNewDoc,
                { line: tabDragger.fromTabs.sectionInfo.lineStart, ch: 0 } as EditorPosition,
                { line: tabDragger.fromTabs.sectionInfo.lineEnd, ch: tabDragger.fromTabs.activeView?.editor.getLine(tabDragger.fromTabs.sectionInfo.lineEnd).length } as EditorPosition);
              tabDragger.toTabs.sectionInfo.lineStart = tabDragger.toTabs.sectionInfo.lineStart - tabDragger.draggedContentLineCount;
              tabDragger.fromTabs.sectionInfo.lineEnd = tabDragger.fromTabs.sectionInfo.lineEnd - tabDragger.draggedContentLineCount;
              ...
              activeView.editor.replaceRange(toTabsNewDoc,
                { line: tabDragger.toTabs.sectionInfo.lineStart, ch: 0 } as EditorPosition,
                { line: tabDragger.toTabs.sectionInfo.lineEnd, ch: tabDragger.toTabs.activeView?.editor.getLine(tabDragger.toTabs.sectionInfo.lineEnd).length } as EditorPosition);
              tabDragger.fromTabs.sectionInfo.lineStart = tabDragger.fromTabs.sectionInfo.lineStart + tabDragger.draggedContentLineCount;
              tabDragger.toTabs.sectionInfo.lineEnd = tabDragger.toTabs.sectionInfo.lineEnd + tabDragger.draggedContentLineCount;
            }
          }
